  ...
  return (
    <PatientProvider>
      <div className="min-h-screen bg-background text-foreground flex">
        ...
        {/* ---- SIDEBAR ---- */}
        <aside
          className={`fixed lg:sticky top-0 left-0 z-50 h-screen w-[72px] bg-card border-r border-foreground/10 flex flex-col items-center py-6 gap-2 transition-transform duration-300 ${sidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"
            }`}
        >
            ...
            {sidebarItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  href={item.href}
                  title={item.label}
                  className={`w-10 h-10 flex items-center justify-center transition-all ${isActive
